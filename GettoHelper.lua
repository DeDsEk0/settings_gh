...
function main()
    ...
    if not doesFileExist(getWorkingDirectory .. 'config/gh_logo') then
        downloadUrlToFile(script_url, script_path, function(id, status)
            if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                sampAddChatMessage("Изображение для скрипта скачалось! Перезагрузка...", -1)
                thisScript():reload()
            end
        end)
    end
    ...
    notf.addNotification('GettoHelper Loaded! Version:'..lastver..'. Author: Lill_Chich', 5)
    menu = 4
